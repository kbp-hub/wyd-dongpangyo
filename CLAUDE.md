...
- Supabase Storage: 자료실 파일 업로드/다운로드
- 배포: Vercel

## 페이지 구성 (헤더 메뉴 순서)
1. `/` - 메인 (카운트다운, 대회일정 요약, 바로가기, 최신소식, 홈스테이 CTA)
2. `/about` - WYD 소개 (WYD 개념, 대회 안내+일정 통합, 주제 성구, 한국가톨릭, WYD 상징물)
3. `/homestay` - 홈스테이 안내 (가정 모집, FAQ, 신청 CTA)
4. `/homestay/apply` - 홈스테이 신청서 (Google Apps Script 연동)
5. `/programs` - 본당 프로그램 (준비 중)
6. `/participate` - 봉사자 모집 (9개 분야)
7. `/join` - 영성 참여 (십자가·성모성화 순례, 묵주기도, 영성운동)
8. `/resources` - 자료실 (Supabase Storage)
9. `/news` - 소식/공지
10. `/schedule` - 일정/장소 (헤더 제거, URL 직접 접근 가능)
